...
test("Should be thenable", async () => {
	...
	frontier.seedURL("http://example.com").then(result1Fn);
	await frontier.runToCompletion();

	// assert
	expect(result1Fn).toBeCalledTimes(1);
	expect(result1Fn).toBeCalledWith({
		cached: false,
...
test("Should Cache correctly", async () => {
	...
	const frontier = new URLFrontier();
	frontier.seedURL("http://example.com").then(result1Fn);
	frontier.seedURL("http://example.com").then(result2Fn);
	frontier.seedURL("http://example.com").then(result2Fn);
	frontier.seedURL("http://example.com").then(result2Fn);
	await frontier.runToCompletion();
	...
	expect(result2Fn).toBeCalledWith({
		cached: true,
		url: "http://example.com/",
	});
});
...
test("Should handle multiple calls and looping", async () => {
	...
	// use
	const frontier = new URLFrontier();
	frontier.seedURL("http://example.com").then(result1);
	frontier.seedURL("http://foo.example.com").then(result2);
	frontier.seedURL("http://foo.example.com").then(result3);
	frontier.seedURL("http://foo.example.com/1234567").then(result4);
	frontier.seedURL("http://bar.example.com").then(result5);
	...
	expect(result2).toBeCalledWith({
		cached: false,
		url: "http://foo.example.com/",
	});
	expect(result3).toBeCalledTimes(1);
	expect(result3).toBeCalledWith({
		cached: true,
		url: "http://foo.example.com/",
	});
	expect(result4).toBeCalledTimes(1);
	expect(result4).toBeCalledWith({
		cached: false,
		url: "http://foo.example.com/1234567",
	});
	expect(result5).toBeCalledTimes(1);
	expect(result5).toBeCalledWith({
		cached: false,
...
test("Should call uncached before cached", async () => {
	...
	const p2 = frontier.seedURL("http://example.com").then(() => 2);
	for await (const url of frontier) {
		urls.push(url);
	}
...
test("Should call earlier before later", async () => {
	...
	const frontier = new URLFrontier();
	const p1 = frontier.seedURL("http://example.com").then(() => 1);
	const p2 = frontier.seedURL("http://foo.example.com").then(() => 2);
	await frontier.runToCompletion();
...
test("Should call different hosts on time", async () => {
	...
	const frontier = new URLFrontier();
	const p1 = frontier.seedURL("http://example.com").then(() => 1);
	const p2 = frontier.seedURL("http://example.com/1234").then(() => 2);
	const p3 = frontier.seedURL("http://foo.example.com").then(() => 3);

	// assert
	// Do some asserts with .toBe and Promise.race
	expect(Promise.race([p1, p2, p3])).resolves.not.toBe(2);
	expect(Promise.race([p2, p3])).resolves.toBe(3);
	await frontier.runToCompletion();
});

test("Should run to completion after sendURL", async () => {
	// use
	const frontier = new URLFrontier();
	const p1 = frontier.seedURL("http://example.com").then(() => 1);
	const p2 = frontier.seedURL("http://example.com/1234").then(() => 2);
